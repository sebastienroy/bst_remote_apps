""" 
    This tool is intended to be interfaced with the Baby Shutter Tester measurement tool.
...
def testMeasureThread():
    ...
    currentId = 0
    entries = [
        '{ \"version\": \"1.0.0\", \"eventType\": \"Measure\", \"measureMode\": \"unitary\", \"effectiveTime\": 36002, \"totalTime\": 58472, \"sum\": 126008, \"min\": 1813, \"illumination\": 14, \"relativeSignal\": 0.007663, \"maxRelativeSignal\": 0.007663, \"parameters\": { \"samplingPeriod\": 4, \"noiseAmplitude\": 0, \"baseSignal\": 1827, \"threshold\": 5 } }',
        'pof pof', 
         ''] 
...
def confidence(signal: float)-> Literal["high", "medium", "low"]:
    if signal >= 0.05:
        return "high"
    elif signal >= 0.01:
        return "medium"
...
def deviation(speedSetting: int, effectiveTime) -> float:
    correspondances = { 15: 16.0, 
                       30: 32.0, 
                       60: 64.0, 
                       125: 128.0, 
                       250: 255.0, 
    ...
    exactSetting = correspondances.get(speedSetting, speedSetting)

    timeSetting = 1000000. / exactSetting
    return  math.log(timeSetting, 2) - math.log(effectiveTime, 2)
...
class TreeviewEdit(ttk.Treeview):
    ...
    def update_cell(self, event):
        ...
        # Do not forget to also update dependencies : the deviation
        new_deviation = deviation(int(new_text), float(app.document[int(selected_iid)]['effectiveTime']))

        current_values = self.item(selected_iid).get("values")
        current_values[column_index-1] = new_text
        
        deviation_index = self['columns'].index('deviation')
        # a better code would get the format from the DataDef...
        current_values[deviation_index] = '{:0.1f}'.format(new_deviation)
        app.document[int(selected_iid)][SPEED_SETTING_ID] = new_text
        app.document[int(selected_iid)]['deviation'] = new_deviation
...
class RemoteApp:
    ...
    def documentToLists(self):
        ...
        data = []
        # Metadata
        data.append(["Baby Shutter Tester mkII data exported using Remote Application"])
        data.append(["https://github.com/sebastienroy/bst_remote_apps"])
    ...
    def run(self):
        ...
        self.ws.title("Baby Shutter Tester mk II Remote Application")
